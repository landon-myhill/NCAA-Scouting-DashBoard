...
from core import HISTORY_DIR as H, PLAYERS_FILE, SEASON_YEAR
from core.config import STARS_FILE
...
for f in [PLAYERS_FILE] + sorted(H.glob("players_*.json")):
    d = json.load(open(f, encoding="utf-8"))
    y = SEASON_YEAR if f.name == "players.json" else int(re.search(r"(\d{4})", f.name).group(1))
    for p in d["players"]:
        p["_year"] = y
        key = norm(p["name"])
...
stars = json.load(open(STARS_FILE, encoding="utf-8"))
