...
def assign_ids_ranks(players: list[dict]) -> None:
    """Assign stable identity ids + positional rank/tier in place.

    `players` must already be sorted best-first. ids come from name+school so
    they persist across scrapes; on the rare hash collision we perturb the
    loser's id deterministically so the id->player map stays 1:1.
    """
    used: set[int] = set()
    for i, p in enumerate(players):
        pid = stable_id(p.get("name", ""), p.get("school", ""))
        while pid in used:
            pid = (pid * 2654435761 + 1) & 0x7FFFFFFF  # deterministic re-probe
        used.add(pid)
        p["id"] = pid
        p["rank"] = i + 1
...
_assign_ids_ranks = assign_ids_ranks
...
def scrape_year(year: int, out_file: Path, skip_if_exists: bool = False,
    ...
    # Sort by composite draft score, assign STABLE ids + positional rank/tier.
    # id is derived from identity (name+school) so it survives re-scrapes —
    # SQLite notes/watchlist keep pointing at the same player. rank is positional.
    for p in all_players:
        p["draft_score"] = draft_score(p)
    all_players.sort(key=lambda p: p["draft_score"], reverse=True)
    _assign_ids_ranks(all_players)

    # Fail loudly if sports-reference changed its layout and we scraped garbage.
    try:
        validate_players(all_players)
